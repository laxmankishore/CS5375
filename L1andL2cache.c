...
int ifDataInCacheL1(uint64_t address, int n_way, struct L1Cache *l1)
{
...
int ifDataInCacheL2(uint64_t address, int n_way, struct L2Cache *l2)
{
    uint64_t block_addr = address >> (unsigned)log2(64);
...
void WritingIntoL1Cache(uint64_t address, int n_way, struct L1Cache *l1)
{
    uint64_t block_addr = address >> (unsigned)log2(64);
    int setNumber = block_addr % 512;
    uint64_t tag = block_addr >> (unsigned)log2(512);
...
void WritingIntoL2Cache(uint64_t address, int n_way, struct L2Cache *l2)
{
...
int main(int argc, char *argv[])
{
    ...
    if (strncmp(argv[1], "direct", 6) == 0)
    {
        while (fgets(mem_request, 20, fp) != NULL)
        {
            ...
            int dataInL1 = ifDataInCacheL1(address, l1nway, &l1);
            if (dataInL1 == 1)
            {
                l1.hits++;
                l2.hits++;
            ...
            {
                l1.misses++;
                int dataInL2 = ifDataInCacheL2(address, l2nway, &l2);
                if (dataInL2)
                {
                    l2.hits += 1;
                }
                ...
                {
                    l2.misses++;
                    WritingIntoL2Cache(address, l2nway, &l2);
                }
